...
import DetailPage from "@/components/product-page/Header/index";
import Tabs from "@/components/product-page/Tabs/index";
import { notFound } from "next/navigation";
import { Product } from "@/interface";

export default async function ProductPage({ params }: any) {
  // Fetch the main product data
  const productQuery = `*[_type == "product" && _id == "${params.slug}" ][0]{
  ...
  const relatedProductsQuery = `*[_type == "product" && category == "${productData.category}" && _id != "${productData.id}"][0...4]{
